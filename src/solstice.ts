...
export type SolsticeData = {
  ...
  winter: number[]; // Northern winter (December) solstices as Julian days
};
...
export function getSolstices(startJD: number, endJD: number): SolsticeData {
  ...
  const summer = getSunPhases(startJD, endJD, 0.25);

  // Calculate winter solstices (December) at phase 0.75
  const winter = getSunPhases(startJD, endJD, 0.75);
